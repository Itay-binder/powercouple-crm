...
export default function OrdersPipelinesTab() {
  ...
  const [newStages, setNewStages] = useState("שלב א,שלב ב");
  ...
  async function createPipeline() {
    ...
    try {
      const res = await fetch("/api/opportunities/pipelines", {
        ...
        body: JSON.stringify({ name, stages, scope: "moving_order" }),
      });
      const j = (await res.json()) as { ok?: boolean; error?: string };
      if (!res.ok || !j.ok) throw new Error(j.error ?? "נכשל");
      setNewName("");
      setNewStages("שלב א,שלב ב");
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      >
        <div style={{ fontWeight: 900, marginBottom: 10 }}>פייפליין חדש (הזמנות)</div>
          ...
          <tbody>
            {pipelines.map((p) => (
              <tr key={p.id}>
                ...
                <td style={{ padding: 12, borderTop: "1px solid #f3f4f6", fontSize: 13, color: "#4b5563" }}>
                  {editingId === p.id ? (
                    <input
                      value={editStages}
                  ...
                  {editingId === p.id ? (
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button
                        ...
                        style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "#059669", color: "#fff", fontWeight: 700 }}
                      >
                        שמור
                      </button>
                      <button type="button" onClick={() => setEditingId(null)} style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #d1d5db" }}>
                        בטל
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button
                        type="button"
                        onClick={() => startEdit(p)}
                        style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #c4b5fd", background: "#f5f3ff", fontWeight: 700 }}
                      >
                        עריכה
                      </button>
                      <button
                        ...
                        style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #fecaca", background: "#fff1f2", color: "#9f1239", fontWeight: 700 }}
                      >
                        מחק
                      </button>
                    </div>
